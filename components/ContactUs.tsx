...
import { CONTACT_CONTENT } from '../constants/landing-page/contact';
...
export const ContactUs = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          access_key: CONTACT_CONTENT.form.web3FormsAccessKey,
          name: formData.name,
          email: formData.email,
          message: formData.message,
          subject: `New Contact Form Submission from ${formData.name}`,
          from_name: 'Feedback PWA Landing Page',
          to_email: CONTACT_CONTENT.form.toEmail,
        }),
      });
  ...
  return (
    <section
      id='contact'
      className='py-24 relative overflow-hidden bg-gradient-to-b from-surface to-background'
    >
      {/* Decorative Elements */}
          ...
          <div>
            ...
            <div className='space-y-6'>
              <div className='flex items-start gap-4 group'>
                ...
                <div>
                  <h3 className='text-foreground font-semibold mb-1'>
                    {CONTACT_CONTENT.info.email.title}
                  </h3>
                  <EmailLink email={CONTACT_CONTENT.info.email.address} className='text-muted text-sm hover:text-primary transition-colors' />
                </div>
              </div>
              ...
              <div className='flex items-start gap-4 group'>
                ...
                <div>
                  <h3 className='text-foreground font-semibold mb-1'>
                    {CONTACT_CONTENT.info.visit.title}
                  </h3>
                  <p className='text-muted text-sm'>
                    {CONTACT_CONTENT.info.visit.address}
                  </p>
                </div>
              </div>
              ...
              <div className='flex items-start gap-4 group'>
                ...
                <div>
                  <h3 className='text-foreground font-semibold mb-1'>
                    {CONTACT_CONTENT.info.call.title}
                  </h3>
                  <p className='text-muted text-sm'>{CONTACT_CONTENT.info.call.number}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className='bg-surface/50 backdrop-blur-xl border border-white/10 rounded-2xl p-8 lg:p-10 shadow-2xl'>
            <form onSubmit={handleSubmit} className='space-y-6'>
              <div>
                <label
                  htmlFor='name'
                  className='block text-sm font-medium text-muted mb-2 tracking-wide'
                >
                ...
                <span className='relative z-10 flex items-center justify-center gap-2'>
                  {isSubmitting ? (
                    <>
                      ...
                      {CONTACT_CONTENT.form.submittingText}
                    </>
                  ) : (
                    <>
